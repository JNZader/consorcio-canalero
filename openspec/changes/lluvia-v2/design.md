...
`POST /geo/rainfall/scopes:resolve`, `POST /geo/rainfall/analyses`, and `GET /geo/rainfall/analyses/{revision}.csv` require `require_admin_or_operator`, Origin/content-type CSRF middleware and rate limiting. JSON/CSV share one metric-row serializer over the snapshot; unauthorized responses reveal no values.
