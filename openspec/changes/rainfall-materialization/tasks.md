...
- [x] 3.19 Register `compute.py` in the rainfall block of `.cosmic-ray.toml`, kept commented/unmeasured (repo rule: no threshold gate without a measured run — blocked on the same Python-3.11 cosmic-ray availability gap already tracked for `policy.py`/`service.py`/`temporal.py`; no new numeric threshold is invented here). Files: `gee-backend/.cosmic-ray.toml`.

## Phase 4 (PR 4) — Daily-Source Flip

- [x] 4.1 `RAINFALL_DAILY_SOURCE` flips `"sqpe-obs"` → `"chirps-v3-sat"` with a `TODO(smn)` comment; provenance sets `fallback_used=True` for any role whose spec-primary source differs from the one actually used (documents the "Daily role uses the documented default" spec scenario as a deliberate, tracked deviation). Test: `test_rainfall_materialization.py::test_daily_source_flips_to_chirps_v3_sat_with_fallback_flag`. Files: `gee-backend/app/domains/geo/rainfall/service.py` (constant + `RAINFALL_SPEC_PRIMARY_SOURCE_BY_ROLE` + `fallback_used_for`), `gee-backend/app/domains/geo/rainfall/tasks.py` (threads `fallback_used_for(row.role, row.source_id)` into `build_snapshot` inside `_persist_analysis_revision`, needed to actually observe the flag on a persisted snapshot). Also updated the pre-existing daily-routing pin (`test_ingest_ops.py::test_resolve_missing_work_source_uses_daily_for_current_year`, genuine RED confirmed before the flip) and removed two now-redundant `RAINFALL_DAILY_SOURCE` monkeypatches that pre-empted this flip in PR3's E2E tests (`test_e2e_same_key_later_date_and_corrected_revision_becomes_visible`, `test_e2e_year_rollover_transitions_to_final`) — `sqpe-obs`'s `NotImplementedError` contract (`test_provider_adapters.py:436-456`) stays unmodified and valid.
- [x] 4.2 First real current-year materialization: with the flip live, a current-year key reaches `done` (no longer blocked by the unwired `sqpe-obs` adapter) and the stage-1 daily sweep starts finding keys. Test: `test_rainfall_materialization.py::test_current_year_key_reaches_done_after_flip_and_sweep_finds_it`. Files: none (verifies PR 1-3 under the flipped constant).
