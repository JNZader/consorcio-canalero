...
| Unit | hoist ignores provenance-less metrics (D5) | a fixture where one metric is served in the stripped four-field shape and the rest are identical → the shared block still hoists all nine fields, the stripped metric renders state + reason only, and no provenance line is invented for it |
