...
Six preconditions, each stated because each is a way to get a dishonest green (the first three were the original list; 4 and 5 were surfaced as UXJA-201 in the final re-judge, and 6 was proved by the first executed closeout): (1) the catastro dataset must be loaded (the same precondition `ficha-territorial.spec.ts` has always had — without it the tile gate skips); (2) `E2E_API_BASE` is what stops the preamble from soft-skipping; (3) `E2E_APP_URL` is what turns `requireCondition` from a skip into a failure; (4) **`FICHA_ENABLED=true` must reach the backend service** — `app/config.py:124` defaults it to `False` and no compose/env file in the repo sets it, so without it `probeFichaAvailability` returns `'off'` and every test skips; (5) **Martin must be reachable from the browser's host** — the compose service is docker-network-only (no published port, `docker-compose.yml` martin block), while the SPA resolves tiles from `VITE_MARTIN_URL || 'http://localhost:3000'`, so the run needs either a compose override publishing `3000` or `VITE_MARTIN_URL` pointed at a reachable Martin; without it `clickFixtureParcela` never gets its `parcelas_catastro` 200 and the zero-scroll case skips; (6) **`suelos_catastro` must be populated** — the ficha computes soils before the rainfall tab can reach `ficha-result`, and an empty table returns `503 dataset_no_cargado`, so a live catastro tile alone is insufficient. The port matters — `run dev` serves 5173, while `run preview` serves 4173, so a preview run needs `--port 5173` or a matching `E2E_APP_URL`. Run with no env vars at all, the script skips loudly and touches nothing: the spec navigates to `APP_URL`, which defaults to `localhost:5173`, never to the config's production `baseURL`.

**Executed closeout evidence (2026-08-15).** An isolated stack used frontend `15174`, backend `18001`, Martin `13001`, PostgreSQL `15433` and Redis `16380`; `FICHA_ENABLED=True`, `parcelas_catastro=1322`, fixture `3603003210041000=1`, `suelos_catastro=45`, and Martin's host catalog exposed `parcelas_catastro`. The exact named run with matching `E2E_APP_URL` / `E2E_API_BASE` executed **10/10 passing, 0 skipped, exit 0**. At 390×844 / `medio`, the complete card measured top `669.390625`, bottom `792.671875`, height `123.28125`; the visible body measured top `613.796875`, bottom `808.59375`, height `194.796875`, leaving `15.921875` px below the card with `scrollTop=0`.
