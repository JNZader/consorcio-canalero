...
- [ ] A3a.2 Cheap `poligono` schema validators only: vertex count, ring count, coordinate sanity. AC: "Vertex cap rejected before raster read". (~30)
...
- [ ] A3a.4 Body-size guard on the ficha router: reject `Content-Length > ficha_max_body_bytes` with 413 `cuerpo_excedido` before parsing; chunked bodies read through a counting guard. AC: "Oversized body rejected before parsing" (JDB-007). (~50)
- [ ] A3a.5 `gee-backend/app/config.py`: add `ficha_rate_limit_*`, `ficha_max_area_ha` (20 000), `ficha_max_envelope_ha` (60 000), `ficha_max_vertices` (1 000), `ficha_max_buffer_m` (2 000), `ficha_max_body_bytes` (1 MiB), `ficha_max_concurrency` (4), `ficha_low_confidence_pixel_ratio` (10). (~25)
...
- [ ] A3a.10 **Ledger-mandated route-table guard test** (JD-A-010, judge-forced): walk `app.routes` and assert `analisis-zona` is the ONLY `/api/v2/geo` route without an operator dependency. AC: `geo-analysis-endpoint` › "No auth regression on sibling routes". (~30)
