...
- [x] 4.1 RED→GREEN: `gee-backend/tests/new/conocimiento/test_rag_metrics.py::test_hit_rate_at_5_and_mrr` + `test_citation_precision_norma_secundaria_vigencia_correctness` — create `gee-backend/app/domains/conocimiento/eval/metrics.py`: pure set-comparison functions against gold citation keys, no LLM-as-judge. The spec fixes the BARS and never defined the metrics, so the definitions are fixed in `metrics.py` and in design.md D6 (citation-precision = R-precision; see apply-progress deviation #2).
- [x] 4.2 RED→GREEN: `gee-backend/tests/new/conocimiento/test_rag_abstention.py::test_loocv_selection_differs_from_same_sample_fit` + `test_loocv_fallback_counted_when_no_threshold_reaches_recall_1` — create `gee-backend/app/domains/conocimiento/abstention.py`: `AbstentionPolicy(min_score, min_margin, require_both_legs)` + `select_threshold_loocv()` (per-mode leave-one-out; tie-break highest precision at recall 1.00, ties by lower threshold; no-threshold-reaches-1.00 fold-back counted, not silently dropped).
- [x] 4.3 RED→GREEN (CRA-101/CRB-101 fold): `test_rag_abstention.py::test_abstention_denominator_from_gold_set_not_literal` — the unanswerable-count denominator reads `gold_set.yaml`'s `clase=='unanswerable'` count, never a hardcoded `18`. design.md D5's own literal `18` corrected to the ratified 23 in the same pass.
- [x] 4.4 Draft the gold-set source — RESOLVED DIFFERENTLY: O.2 closed before this slice ran, so the draft already existed and was ratified verbatim. What this task produced instead is `docs/rag/gold-set-provenance.md`: the traceability record (source, ratification, the 29/23 composition, the two caveats the draft raises about itself, the one key corrected against the corpus). The ratified draft is NOT copied into this repository — see deviation #1.
- [x] 4.5 Create `gee-backend/app/domains/conocimiento/eval/gold_set.yaml` — 52 items, `validado_por: owner` throughout (O.2 ratified verbatim: 29 answerable + 23 unanswerable). Schema as specified plus `subclase`/`citas_vigencia`/`origen`. The 26 items transcribed from `clasificacion='privado'` documents carry `pregunta_ref` and resolve from `RAG_GOLD_PRIVADO_PATH` — deviation #1.
- [x] 4.6 RED→GREEN: `gee-backend/tests/new/conocimiento/test_rag_eval_harness.py::test_n_lt_20_blocks_scoring` — create `gee-backend/app/domains/conocimiento/eval/harness.py` (`--mode fts|vector|hybrid`, one code path); refuses go/no-go unless every item is `owner`-validated and `answerable` ≥ 20. An unresolved private item blocks it too, so the privacy split cannot quietly shrink the set.
- [x] 4.7 RED→GREEN: `test_rag_eval_harness.py::test_three_modes_same_questions_separate_metric_blocks` (pgvector-marked, plus an unmarked single-mode shape so CI covers the block structure) + `test_unanswerable_question_triggers_abstention` + `test_answerable_question_above_threshold_returns_hit`.
- [x] 4.8 RED→GREEN: `test_rag_eval_harness.py::test_vigencia_trap_surfaces_true_current_state` — Ley 8548 / Ley 10679-art.20-post-2023 adversarial questions return the true current `estado_vigencia`, not the historic text.
- [x] 4.9 RED→GREEN: `test_rag_eval_harness.py::test_false_confident_answer_fails_go_no_go_regardless_of_other_metrics` + `test_same_sample_fit_cannot_carry_go_no_go` — go/no-go decided ONLY on the LOOCV-held-out pair (recall 1.00 AND precision ≥ 0.80); same-sample figure labeled `upper bound (fit on the scoring sample)` wherever reported. Each `Barra` carries its own `fuente`, so a report cannot mislabel which number decided.
- [x] 4.10 RED→GREEN: `gee-backend/tests/new/conocimiento/test_rag_privacy.py::test_api_baseline_gated_by_assert_public_domain` + `test_private_document_excluded_from_external_payload` — `assert_public_domain(db, corpus_sha)` default-deny gate, plus a second independent guarantee (the payload SQL selects `clasificacion='publico'` only). O.5 resolved to a local baseline, so V0 still makes zero external calls.
- [x] 4.11 Create `gee-backend/app/domains/conocimiento/eval/report.py` — writes `docs/rag/retrieval-eval-{sha8}-{YYYY-MM-DD}.md` + `results.json`; per-mode methodology-disclosure block; provenance read from `rag_corpus` (migration `conocimiento_004`), never from a sidecar; REFUSES to render on a `sintetico` snapshot. Corpus-side torch/device are reported as `no registrado en la base` — ledger RAG4-002.
- [x] 4.12 Create `gee-backend/scripts/rag_eval.py` (wires harness+metrics+abstention+report); modify `Makefile` — add `rag-ingest`, `rag-embed-load`, `rag-eval` targets delegating to the Slice 2–4 scripts. `main()` covered by `test_rag_eval_cli.py` against real PostgreSQL (the RAG2-006 lesson: an untested `main()` is where the pieces stop fitting).
- [x] 4.13 Register `parser.py`/`fusion.py`/`abstention.py`/`eval/metrics.py` in `gee-backend/.cosmic-ray.toml` as a **commented, unmeasured** `module-path` block — precedent: the rainfall entries. Carries the test files, their case counts and the exact measurement command. NOT wired to CI (repo rule: no numbers, no gate).
- [ ] 4.14 E2E verification (no new file): run `make rag-eval` once ingestion (Slice 2 + O.1), embeddings (Slice 3 + O.3), and the owner-curated gold set (O.2) all exist; confirm the report lands under `docs/rag/` and is reviewed by the owner (proposal Success Criteria). **BLOCKED on O.3** (the RTX batch run) — the exact owner command sequence is in apply-progress.md § "The owner command sequence".

### Slice 4 ledger amendments (from the slice-3 scoped re-review)

- [x] A1 (RAG3-R01) `design.md` D4: the `vector_search()` bullet credited `EmbeddingsNoCargadas` and `EmbedderMismatch` to the repository leg. False — the leg receives a query VECTOR, not an embedder, and raises only `VectorSupportUnavailable`; both provenance refusals live in `service.verificar_embedder`. Attribution corrected, and the warning behind it honored: a new stated design rule that every retrieval consumer goes through `service.recuperar`, asserted by `TestServiceLayerBoundary` over the eval package's real import graph AND behaviourally by a spy on the call.
- [x] A2 (RAG3-R02) apply-progress's slice-3 corpus-shape line read `2118 passed, 19 skipped` with the failure only counted inside the following sentence's `+1`, so the table alone read 2137 against the CI shape's 2138. Re-measured on this tree at `90d4acf` before any slice-4 change — CI shape **2114 passed / 59 skipped**, corpus shape **2144 passed / 29 skipped**, both exit 0 — the record corrected in place, and every shape line now states passed / skipped / failed explicitly.

### Slice 4 fix round (reliability lens + general refuter on the slice-4 diff)

- [x] G1 **RAG4-001 CRITICAL (refuter STANDS)** — `repository.FTS_SEARCH_SQL` now builds a DISJUNCTION of the lexemes `websearch_to_tsquery` parses, with exclusions preserved and the parse round-tripped through `CAST(… AS tsquery)` so the Snowball stemmer is not applied twice. Measured against the pinned corpus, six of six sampled gold questions went from **0 rows** (gold in the candidate set for **zero** of six) to a full 50-candidate leg with the gold key inside the set for four of six. Operator named in `FTS_OPERADOR`, surfaced in the report markdown, in the JSON and beside the coverage counters. `hibrido_degenerado` re-thresholded from `all(...)` to `leg_*_degradada`'s strict majority. Pinning tests flipped to the new semantics + `TestFtsOperador` (12 cases: double-stemming, exclusions, empty/stopword/only-exclusion degradation, and ten hostile inputs). `design.md` D4 amended with the operator decision, the measurement table and the construction's three properties.
- [x] G2 **RAG4-003 CRITICAL (refuter STANDS)** — `harness.verificar_corpus_sha` refuses when `gold_set.yaml`'s `corpus_sha` differs from the snapshot being evaluated, and `_textos_privados` verifies the owner-side file's `para:` and `corpus_sha:` before its 26 questions are allowed to resolve. `rag_eval.main` exits **4** naming both SHAs, before the database is opened, before the report directory exists and before an embedder is built. `gold_corpus_sha` added to the report JSON.
- [x] G3 **RAG4-004 WARNING→promoted** — `metrics.separacion_norma_secundaria` returns `None` for an empty page instead of `1.0`. It was the only metric that awarded perfection to total retrieval failure, into a hard `== 1.00` bar: a mode that retrieved nothing cleared it unanimously. Denominator exposed as `n_separacion` in the dataclass, the JSON and a new `n` column in the report's metric table.
- [x] G4 **RAG4-005 WARNING→promoted** — the A1 import boundary had two holes: `ImportFrom` only inspected `node.module`, so `from app.domains.conocimiento import repository` passed (verified: old predicate `[]`, new predicate catches it), and `importlib` was unguarded. Both closed, plus the behavioural spy now also asserts `repository.vector_search` received **no** call. RED-probed by planting both bypasses in `eval/privacy.py`.
- [x] G5 **RAG4-006 WARNING→promoted** — a fused mode's verdict states its scope when a leg is degraded (`GO (con leg FTS degradada — el veredicto refleja principalmente la pata vectorial)`), with `veredicto_calificado` and `legs_degradadas` in the JSON. A degraded leg does NOT force `evaluable=False`: the ablation still informs.
- [x] G6 **RAG4-007 SUGGESTION→promoted** — `rag_eval._embedder` is spied in the CLI tests and asserted never called on the three refusal paths (unknown snapshot, SHA mismatch, synthetic snapshot), each exercised with the DEFAULT three-mode ablation so an embedder would genuinely be built. Removes the torch-absence environment coupling that made 4.12's ordering property pass for the wrong reason.
...
- [x] O.2 **Blocks the eval RUN (4.14), not the Slice 4 PR.** Owner validates/curates `docs/rag/eval-preguntas-oro-DRAFT-2026-08-10.md` (4.4) into `gold_set.yaml` (4.5): set `validado_por: owner` on every item, reach `answerable` ≥ 20. **RESOLVED** in owner round 2 (see below): ratified verbatim at 29 answerable + 23 unanswerable = 52.
...
- [x] O.4 Owner call: CX33 latency-measurement approach — throwaway on-box container vs. cpuset-matched local proxy labeled `ESTIMATE`. **RESOLVED** in owner round 2 (measure locally, labelled; on-box deferred to the V1 gate) **and now SHIPPED** (JD round 1, RJDA-006): the resolution had reached no code, so the criterion existed and the deliverable did not carry it. `scripts/rag_query_latency.py` already stamped `etiqueta` (LOCAL / ESTIMATE) into a JSON nobody read; `rag_eval.py --latencia` reads it and the report renders a latency block in markdown and JSON with the label, p50/p95, and the core/thread/device conditions beside them. Absent, the report states that the criterion was not evaluated rather than omitting the section.
- [x] O.5 Owner call: hosted-embedding baseline provider — Voyage-3 vs OpenAI `text-embedding-3-large` truncated to 1024 dims. **RESOLVED** in owner round 2 (neither: a LOCAL multilingual-e5-large baseline, zero external calls) **and now SHIPPED** (JD round 1, RJDA-005/RJDB-005): the resolution had reached `requirements-rag.txt` and `--embedder`'s `choices` but never `get_embedder`, so the option argparse accepted raised `ValueError: unknown embedder 'e5-large'`. `E5Embedder` implements it over sentence-transformers with the REQUIRED `query: `/`passage: ` role — E5 is asymmetric and the wrong prefix degrades retrieval without raising — and carries its own 512-token ceiling, which the pre-flight now reads off the embedder instead of assuming BGE-M3's 8192.
