...
| Partial failure / recovery | Tested: `test_unresolvable_head_does_not_apply_partial_state` proves a bad target revision leaves the throwaway DB with zero conocimiento tables (no half-migrated state). The D7 stranded-volume recovery was documented and, per design.md itself, "never exercised by CI" — and it was WRONG on both counts it claimed, which is what "documented but never run" buys you (RJDB-003, JD round 1). Both the `downgrade -1` form recorded here and its `downgrade conocimiento_002` successor leave 002 applied, so its `upgrade()` never re-runs, while 003's downgrade deletes rows on the way past: destructive and ineffective at once. All three candidate paths have now been EXECUTED against a throwaway `consorcio-postgres:16-vector` container and the measurements are the table in design.md D7; the recovery is `ddl.UPGRADE_STATEMENTS` executed directly. | Fixed in JD round 1 — the paths are measured, not asserted |
