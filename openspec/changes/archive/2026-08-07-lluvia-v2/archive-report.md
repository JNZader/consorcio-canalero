...
| **Provider adapters (stub)** | Deferred (PR 3A deferral) | `tasks.py:40` raises `NotImplementedError('provider adapter not wired')`; `ResilientAdapter` + outbox retry/circuit machinery in place. Evidence-gated by design: roles activate only after the known-event spike/manifests pass. Staged provider/backfill activation NOT executed in this batch. |
