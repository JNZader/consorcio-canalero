...
- [x] 3.4 **REFACTOR**: Share displayed/exported state formatting without manual memoization; add mutation targets for policy, suppression, temporal windows and CSV parity. DONE in PR 3B + post-archive follow-up (2026-08-07): shared formatting landed as `rainfallFormat.ts` (single formatter for display/export semantics, no manual memoization — React 19). Mutation-target registration: Stryker entries for the rainfall UI slice added to `consorcio-web/stryker.config.mjs` (`src/lib/api/rainfall.ts`, `src/hooks/useRainfallAnalysis.ts`, `src/components/map2d/rainfall/*`); backend target tests written and green — `gee-backend/tests/test_mutation_targets_rainfall.py` (71 cases, 0.50s, policy/service/temporal pure units, finanzas/padron recipe). Backend `gee-backend/.cosmic-ray.toml` entry present but **commented out with `# issue: measure`**: local interpreter is Python 3.14 and cosmic-ray 8.4.6 crashes serializing worker results ("asdict() should be called on dataclass instances"), and the estimated 1015 mutants exceed the 10-min budget; measurement command documented in the toml for the CI 3.11 interpreter. Correction (documented at archive, verify report): the earlier wording "backend policy/suppression/temporal/CSV-parity mutation targets already exist per 2.4" was inaccurate — there were NO rainfall entries at archive time (verified by grep, 0 matches); the follow-up closes this.
