...
| **Higiene de infra + dominio** (batch A CI/tests + Tanda B dominio lluvia + CVE OpenSSL) | #244, #245, #246 | Batch específico CERRADO en main 2026-08-26: #244 parchea el CVE OpenSSL-2026-14456 en la imagen runtime del frontend (fix en Dockerfile; NO hubo rebuild ni deploy en el box), #245 batch A (suite de tests que dice la verdad + CI que conserva sus caches), #246 Tanda B (higiene de dominio lluvia: vocabulario, constantes, orden estable, razones de baseline y resolver de cuencas). Sin ceremonia SSH/box asociada a este batch. |
