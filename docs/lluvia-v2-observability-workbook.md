...
| `rainfall.window_baseline.duplicate_slots` | `tasks.py::_persist_analysis_revision` | The SIBLING of the row above, for the rolling-window reference (`lluvia-antecedente-referencia` D2): `repository.baseline_daily_values` refused to answer because one baseline year holds two non-superseded rows for one `interval_start`. Only the six WINDOW reference metrics (`antecedents.d{7,30,90}_{normal,percentile}`) degrade with `baseline_evidence_invalid`; the antecedent millimetre totals, the annual normal/percentile pair and the workbook normal curve all still build and land. **Expect it to fire WITHOUT its sibling, and read that as information rather than as a bug**: this read spans `[1991-01-01, 2021-01-01)` whole, while `baseline_cumulatives`' windows stop at each baseline year's cutoff, so a duplicate later in a year is invisible to that one and visible to this one. Same evidence, two honest answers. Same repair as above: the missing `rainfall_interval_lifecycle` supersession row, never a delete. | `source_id`, `asset`, `baseline_year`, `matched_rows`, `distinct_slots`, `scope_kind`, `scope_id`, `scope_version`, `year` (the ANALYSIS year being built) |
