...
| `rainfall.series.served` | `router.py::read_analysis_series` | The daily series for one stored revision was returned (HTTP 200). Read-only: this route enqueues nothing, so it can never turn a chart into GEE work. `consistent_with_snapshot` is the **server-side pin** (design.md D3): the revision's `data_revision` digest recomputed over exactly the keys and the D6-widened window the build read, compared with the digest stored on the row. `consistency_reason` is `null` when it matches, `data_revision_moved` when the daily values behind the revision were corrected after it was stored, and `interval_family_ambiguous` when the read rows do not map to exactly one provider-revision family (so the build's own family cannot be reconstructed and no comparison is attempted). **When it spikes**: a rising share of `data_revision_moved` on current-year keys is normal NRT correction traffic — the chart still renders, above its staleness notice — but a key that stays inconsistent means its refresh is not landing (correlate with `rainfall.outbox.cooldown` and `rainfall.outbox.failed`). Any `interval_family_ambiguous` at all is worth a look: it means two live, non-superseded revision families share one window, which decision 7's one-family-per-source invariant says should not happen. | `revision_id`, `data_revision`, `consistent_with_snapshot`, `consistency_reason`, `points`, `latency_ms` |
