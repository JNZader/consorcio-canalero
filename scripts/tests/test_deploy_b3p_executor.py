...
MOUNTS=[{"Type":"volume","Name":"consorcio-backend-cache","Destination":"/app/.cache","RW":True},{"Type":"volume","Name":"consorcio-geo-data","Destination":"/data/geo","RW":True},{"Type":"volume","Name":"consorcio-denuncia-uploads","Destination":"/app/uploads","RW":True}]
class Fake:
    ...
    def run(self, argv):
        ...
        if self.fail=="build-timeout" and "docker compose build backend" in cmd: raise subprocess.TimeoutExpired(cmd, 600)
        if self.fail=="cutover-interrupt" and not self.timed and "docker compose up" in cmd: self.timed=True; raise KeyboardInterrupt()
        if self.fail=="timeout" and not self.timed and "docker compose up" in cmd: self.timed=True; raise subprocess.TimeoutExpired(cmd, 30)
        if self.fail=="rollback-timeout" and "--force-recreate" in cmd: self.live="sha256:old"; raise subprocess.TimeoutExpired(cmd, 30)
        if self.fail=="launch-timeout" and "docker compose run" in cmd: raise subprocess.TimeoutExpired(cmd, 30)
        if self.fail and self.fail in cmd: return 1,"password=no","failed"
        if "docker compose up -d --no-deps --no-build backend" in cmd: self.live="sha256:raced" if self.fail=="live-race" else "sha256:new"
        if "--force-recreate backend" in cmd: self.live="sha256:old"
        if "mktemp /tmp" in cmd: return 0,"/tmp/b3p-compose",""
        if "docker compose build backend" in cmd: self.compose_tag="new"
...
def subject(fake, fresh=None):
    ...
    return Executor(base.DEFAULTS,fake.run,fake.http,fresh or gate,lambda argv: fake.calls.append("TUNNEL "+" ".join(argv)) or object(),lambda _:None,lambda _:None,fake.public),seen
def go(executor): return executor.execute(base.TARGET_SHA,"DEPLOY-B3P",{"CONSORCIO_B3P_DEPLOY_ALLOW_EXECUTE":"1","E2E_ADMIN_EMAIL":"a@b","E2E_ADMIN_PASSWORD":"secret"},"real-basin")
...
def test_compose_image_canary_and_public_phase_contract():
    ...
    assert report.outcome is Outcome.SUCCESS and seen==[base.TARGET_SHA]
    assert report.phases==["lock","preflight","baseline","backup","ff","revalidate","old","build","canary","smoke","cleanup","cutover","post","lock"]
    assert any("docker image inspect consorcio-backend" in c for c in fake.calls)
    canary=next(c for c in fake.calls if "compose run" in c)
...
@pytest.mark.parametrize("failure,outcome",[("docker compose run",Outcome.FAILED),("--force-recreate",Outcome.FAILED_ROLLBACK)])
...
@pytest.mark.parametrize("failure",["docker compose run","build-timeout"])
def test_rejected_pre_cutover_work_restores_and_asserts_mutable_compose_tag(failure):
    ...
    failed=next(index for index,call in enumerate(fake.calls) if failure.replace("build-timeout","docker compose build backend") in call)
    assert len(tags)==2 and failed<restore and not any("docker compose up -d --no-deps --no-build backend" in call for call in fake.calls)
...
def test_report_reasons_and_cli_are_sanitized(monkeypatch,capsys):
    report=go(subject(Fake("docker compose run"))[0])
    ...
    assert module.main(["--target-sha",base.TARGET_SHA,"--confirm",base.CONFIRMATION,"--basin","x"])==2
...
def test_timeout_fence_is_remote_and_requires_stable_post_fence_state():
    fake=Fake("timeout"); report=go(subject(fake)[0]); cutover=next(call for call in fake.calls if "docker compose up -d --no-deps --no-build backend" in call)
...
def test_restore_failure_is_reported_as_unsafe_failed_restore():
    fake=Fake("docker compose run"); executor,_=subject(fake); runner=fake.run; tags=[]
...
def test_existing_lock_refuses_before_fetch_or_mutation(owner):
    fake=Fake(lock_owner=owner); assert go(subject(fake)[0]).outcome is Outcome.FAILED_LOCK and fake.lock_owner==owner and not any(any(word in call for word in ("fetch origin","compose build","docker compose up")) for call in fake.calls)
@pytest.mark.parametrize("failure,outcome",[("",Outcome.SUCCESS),("docker compose run",Outcome.FAILED),("live-race",Outcome.ROLLED_BACK)])
