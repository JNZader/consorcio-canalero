...
class Executor:
    def __init__(self, config=base.DEFAULTS, runner=base._run, http=None, fresh=None, tunnel=None, close_tunnel=None, sleeper=time.sleep, public=None):
        ...
        self.fresh,self.tunnel,self.close_tunnel=fresh or self._fresh,tunnel or self._tunnel,close_tunnel or self._close
    ...
    def _fresh(self, sha, runner):
        ...
        base.verify_github_commit(self.config,sha); base.preflight(self.config,lambda argv:self._invoke(argv,TIMEOUTS["preflight"]))
    @staticmethod
    ...
    def _rollback(self,old,basin,environ,baseline):
        command=" && ".join((f"docker image tag {old} consorcio-backend:rollback-{old.removeprefix('sha256:')}",f"docker image tag {old} {base.COMPOSE_IMAGE}","docker compose up -d --no-deps --no-build --force-recreate backend",f"test \"$(docker inspect --format '{{{{.Image}}}}' consorcio-backend)\" = {old}"))
    ...
    def execute(self,sha,confirmation,environ,basin):
        ...
        try:
            ...
            self._call("ff",f"git -C {self.config.stack} merge --ff-only {sha}")
            self._call("revalidate","docker compose config --quiet")
            base.normalize_mounts(json.loads(self._call("revalidate","docker inspect consorcio-backend --format '{{json .Mounts}}'")))
            ...
            self._call("build","docker compose build backend")
            ...
            self.canary=True
            self._call("canary",f"docker compose run -d --no-deps --no-build --name consorcio-b3p-canary --publish 127.0.0.1:18080:8000 backend python -m app.server")
            ...
            try: self._fenced("cutover","docker compose up -d --no-deps --no-build backend")
...
def main(argv=None):
    ...
    args=parser.parse_args(argv); report=Executor().execute(args.target_sha,args.confirm,os.environ,args.basin)
