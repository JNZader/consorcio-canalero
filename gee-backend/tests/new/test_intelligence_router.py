...
class TestSuggestionEndpoints:
    def test_trigger_canal_analysis(self):
        from app.domains.geo.intelligence.router import trigger_canal_analysis

        mock_task = MagicMock()
        mock_task.id = "sug-task-1"
        with patch(
            "app.domains.geo.intelligence.tasks.run_canal_analysis"
        ) as mock_fn:
            mock_fn.delay.return_value = mock_task
            result = trigger_canal_analysis(_user=MagicMock())
        assert result["task_id"] == "sug-task-1"

    def test_get_results_no_batch(self):
        from app.domains.geo.intelligence.router import get_suggestion_results

        mock_repo = MagicMock()
        mock_repo.get_latest_batch.return_value = None
        result = get_suggestion_results(
            tipo=None, page=1, limit=20,
            db=MagicMock(), repo=mock_repo, _user=MagicMock(),
        )
