...
def test_list_gee_layers_returns_cached_json(client):
    with patch(
        "app.api.v1.endpoints.gee_layers.get_available_layers",
        return_value=[{"id": "zona"}],
    ):
...
def test_sentinel2_tiles_maps_not_found_and_init_errors(client):
    with (
        patch("app.api.v1.endpoints.gee_layers._gee_initialized", False),
        patch(
...
def test_get_caminos_por_nombre_and_generic_layer_error_paths(client):
    with (
        ...
        by_name = client.get(
            "/api/v1/gee/layers/caminos/por-nombre?nombre=CONSORCIO NORTE"
        )
