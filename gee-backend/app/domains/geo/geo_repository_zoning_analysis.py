...
class GeoRepositoryZoningAnalysisMixin:
    def get_active_approved_zoning(
        ...
        stmt = approved_zoning_stmt(active_only=True, cuenca=cuenca).order_by(
            GeoApprovedZoning.approved_at.desc()
        )
        return db.execute(stmt).scalar_one_or_none()

    def list_approved_zonings(
        self, db: Session, *, cuenca: Optional[str] = None, limit: int = 20
    ...
    def get_approved_zoning_by_id(
        self, db: Session, zoning_id: uuid.UUID
    ) -> Optional[GeoApprovedZoning]:
        return db.execute(
    ...
    def get_analisis_list(
        ...
        *,
        ...
        return paginated_results(
            db, base, page=page, limit=limit, order_by=AnalisisGeo.created_at.desc()
        )
