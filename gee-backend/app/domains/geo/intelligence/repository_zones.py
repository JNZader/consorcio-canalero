...
class IntelligenceRepositoryZonesMixin:
    ...
    def _zona_geojson_stmt(self, *, simplified: bool, tolerance: float = 0.0):
        geometry_expr = (
            func.ST_AsGeoJSON(func.ST_Simplify(ZonaOperativa.geometria, tolerance))
            if simplified
            else func.ST_AsGeoJSON(ZonaOperativa.geometria)
        )
        return select(
            ZonaOperativa.id,
            ...
            geometry_expr.label("geojson"),
        )

    def _serialize_zona_feature(
        self, row, *, include_type: bool = True, allow_null_geometry: bool = False
    ) -> dict[str, Any] | None:
    ...
    def get_zonas(
        ...
        *,
        ...
        return self._paginated_results(
            db, base, page=page, limit=limit, order_by=ZonaOperativa.nombre
        )
    ...
    def get_zonas_for_grouping(
        self, db: Session, *, cuenca_filter: str | None = None
    ) -> list[dict[str, Any]]:
    ...
    def get_zonas_criticas(
        ...
        target_niveles = niveles[
            niveles.index(nivel_riesgo_min) if nivel_riesgo_min in niveles else 2 :
        ]
        latest_hci = (
            select(
