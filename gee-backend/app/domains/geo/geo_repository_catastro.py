...
def bulk_upsert_parcelas(db: Session, features: list[dict]) -> tuple[int, int]:
    ...
    for feature in features:
        ...
        try:
            db.execute(_UPSERT_PARCELA_SQL, {
                "nomenclatura": str(nomenclatura), "geom_json": json.dumps(geometry), "tipo_parcela": props.get("Tipo_Parcela"),
                "desig_oficial": props.get("desig_oficial"), "departamento": props.get("departamento"), "pedania": props.get("pedania"),
                "superficie_ha": superficie_ha, "nro_cuenta": str(nro_cuenta) if nro_cuenta is not None else None, "par_idparcela": props.get("par_idparcela"),
            })
            imported += 1
        except Exception:
            skipped += 1
...
def get_afectados_by_zona(db: Session, zona_id: str) -> dict | None:
    ...
    zona_row = db.execute(text("SELECT id, nombre FROM zonas_operativas WHERE id = CAST(:id AS uuid)"), {"id": zona_id}).mappings().first()
    if zona_row is None:
        return None
    afectados = [dict(r) for r in rows]
    total_ha = sum(a["hectareas"] or 0 for a in afectados)
    return {"zona_id": zona_id, "zona_nombre": zona_row["nombre"], "total_consorcistas": len(afectados), "total_ha": round(total_ha, 2), "afectados": afectados}


def get_afectados_by_flood_event(db: Session, event_id: str) -> dict | None:
    event_row = db.execute(text("SELECT id, event_date FROM flood_events WHERE id = CAST(:id AS uuid)"), {"id": event_id}).mappings().first()
    if event_row is None:
        return None
    flooded_zones = db.execute(text("""
    ...
    zonas_afectadas = [zona_data for row in flooded_zones if (zona_data := get_afectados_by_zona(db, row["zona_id"]))]
    all_consorcista_ids = {a["consorcista_id"] for z in zonas_afectadas for a in z["afectados"]}
    total_ha = sum(z["total_ha"] for z in zonas_afectadas)
    return {"event_id": event_id, "event_date": str(event_row["event_date"]), "total_consorcistas": len(all_consorcista_ids), "total_ha": round(total_ha, 2), "zonas_afectadas": zonas_afectadas}
