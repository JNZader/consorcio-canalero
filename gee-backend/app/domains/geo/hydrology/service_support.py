...
def get_canal_length_for_zona_impl(*, db, text_fn, logger, zona_id: uuid.UUID, zona_geometria: dict[str, Any], superficie_ha: float) -> float:
    ...
    try:
        ...
        if row is None:
            logger.debug("No waterways found in zona %s — using sqrt fallback %.1f m", zona_id, fallback_m)
            return fallback_m
        return float(row)
...
def get_rainfall_intensity_impl(*, db, select_fn, rainfall_model, logger, zona_id: uuid.UUID, fecha_lluvia: date) -> float:
    stmt = (
        select_fn(rainfall_model)
        .where(rainfall_model.zona_operativa_id == zona_id, rainfall_model.date == fecha_lluvia)
        .limit(1)
    )
    record = db.execute(stmt).scalar_one_or_none()
    if record is None:
        logger.debug("No CHIRPS record for zona %s on %s — using fallback 20 mm/h", zona_id, fecha_lluvia)
        return 20.0
    return record.precipitation_mm / 6.0


def get_zona_geojson_impl(*, db, select_fn, st_as_geojson, zona_model, json_module, zona_id: uuid.UUID):
...
def preload_zona_db_data_impl(*, db, zona_ids, select_fn, zona_model, get_zona_geojson, get_canal_length_for_zona, get_rainfall_intensity, fecha_lluvia: date) -> tuple[dict[uuid.UUID, Any], dict[uuid.UUID, dict[str, Any]], list[dict[str, Any]]]:
...
def run_gee_for_zonas_impl(*, zona_db_data, fecha_lluvia: date, get_slope_from_gee, get_ndvi_and_c, logger) -> dict[uuid.UUID, dict[str, Any]]:
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        future_to_zona = {executor.submit(_gee_for_zona, zona_id): zona_id for zona_id in zona_db_data}
...
def finalize_flood_flow_impl(*, db, zona_db_data, gee_results, fecha_lluvia: date, today: date, repository, kirpich_tc, rational_method_q, classify_hydraulic_risk, result_schema, logger):
    results = []
    errors = []
    for zona_id, db_data in zona_db_data.items():
        try:
            ...
            q_val = rational_method_q(gee["c_val"], db_data["intensidad_mm_h"], db_data["superficie_ha"] / 100.0)
            nivel_riesgo, porcentaje = classify_hydraulic_risk(q_val, db_data["capacidad_m3s"])
            payload = {
                "fecha_calculo": today,
                "tc_minutos": tc,
