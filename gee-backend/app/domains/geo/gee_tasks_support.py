    ...
    *, explorer, start_date, end_date, method: str
    ...
    if method in ("fusion", "sar_only"):
        sar_result = explorer.get_sentinel1_image(
            target_date=start_date, days_buffer=buffer_days, visualization="vv_flood"
        )
    ...
    if method == "fusion" and (end_date - start_date).days > 5:
        try:
            resultado["comparison"] = explorer.get_flood_comparison(
...
def build_supervised_classification_stats(
    *, explorer, start_date, end_date, logger, datetime_module, ee_module
) -> dict[str, Any]:
    ...
    collection = (
        ...
        .filterBounds(zona)
        .filterDate(
            (start_date - datetime_module.timedelta(days=days_buf)).isoformat(),
            (start_date + datetime_module.timedelta(days=days_buf)).isoformat(),
        )
...
def build_classification_result(
    *, explorer, start_date, end_date, logger, ee_module, datetime_module
) -> dict[str, Any]:
    resultado: dict[str, Any] = {}
    buffer_days = days_buffer(start_date=start_date, end_date=end_date)
...
def detect_vv_anomalies_impl(
    *, dates: list[str], vv_values: list[float], sigma: float
) -> dict[str, Any]:
    if not vv_values:
...
def build_sar_temporal_result(
    *, explorer, start_date, end_date, scale: int, detect_fn
) -> dict[str, Any]:
    time_series = explorer.get_sar_time_series(
        start_date=start_date, end_date=end_date, scale=scale
    )
    anomaly_result = detect_fn(
        dates=time_series["dates"], vv_values=time_series["vv_mean"], sigma=2.0
    )
    resultado = {
