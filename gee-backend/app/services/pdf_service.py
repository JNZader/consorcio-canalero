...
class PDFService:
    def create_emergency_report(self, data: dict) -> io.BytesIO:
        ...
        header_table_data = [
            [
                ...
                Paragraph(
                    f"INFORME TÉCNICO DE SITUACIÓN<br/>{datetime.now().strftime('%d/%m/%Y %H:%M')}",
                    header_label,
                ),
            ]
        ]
        ...
        story.append(
            Paragraph(
                f"Estado de Cuencas: {data.get('cuenca', 'Zona Consorcio')}",
                title_style,
            )
        )
        ...
        for row in data.get("stats", []):
            ...
            summary_data.append(
                [
                    ...
                    Paragraph(
                        f"<b>{row['estado']}</b>",
                        ParagraphStyle("risk", textColor=risk_color, alignment=1),
                    ),
                ]
            )
        ...
        if data.get("recent_maintenance"):
            ...
            maint_data = [
                ["Fecha", "Infraestructura", "Tarea Realizada", "Estado Final"]
            ]
        ...
        def footer(canvas, doc):
            ...
            canvas.drawString(
                1.5 * cm,
                0.7 * cm,
                "Consorcio Canalero 10 de Mayo - Bell Ville, Córdoba",
            )
    ...
    def create_agenda_pdf(self, reunion: dict, agenda: list) -> io.BytesIO:
        ...
        story.append(
            Paragraph(f"<b>Reunión:</b> {reunion['titulo']}", styles["Normal"])
        )
        story.append(
            Paragraph(f"<b>Fecha:</b> {reunion['fecha_reunion']}", styles["Normal"])
        )
        ...
        line_table.setStyle(
            TableStyle([("LINEBELOW", (0, 0), (-1, -1), 1, colors.grey)])
        )
        ...
        for i, item in enumerate(agenda):
            ...
            if item.get("referencias"):
                ...
                for ref in item["referencias"]:
                    label = ref.get("metadata", {}).get(
                        "label", f"{ref['entidad_tipo']} #{ref['entidad_id'][:5]}"
                    )
    ...
    def create_asset_ficha_pdf(self, asset: dict, history: list) -> io.BytesIO:
        ...
        story.append(
            Paragraph(
                f"FICHA TÉCNICA: {asset['nombre']}",
                ParagraphStyle("T", parent=styles["Heading1"], alignment=1),
            )
        )
        story.append(Spacer(1, 10))
        ...
        data = [
            [
                ...
                asset["tipo"].upper(),
                "Estado Actual:",
                asset["estado_actual"].upper(),
            ],
            [
                "Cuenca:",
                asset["cuenca"],
                "Coordenadas:",
                f"{asset['latitud']}, {asset['longitud']}",
            ],
        ]
        ...
        if not history:
            story.append(
                Paragraph("No hay registros de mantenimiento.", styles["Normal"])
            )
        else:
    ...
    def create_tramite_summary_pdf(self, tramite: dict, avances: list) -> io.BytesIO:
        ...
        story.append(
            Paragraph(
                f"EXPEDIENTE: {tramite['numero_expediente'] or 'S/N'}",
                styles["Heading1"],
            )
        )
        story.append(Paragraph(f"<b>Título:</b> {tramite['titulo']}", styles["Normal"]))
        story.append(
            Paragraph(f"<b>Estado:</b> {tramite['estado'].upper()}", styles["Normal"])
        )
        story.append(Spacer(1, 20))
        ...
        for a in avances:
            story.append(
                Paragraph(
                    f"<b>{a['fecha'][:10]} - {a['titulo_avance']}</b>", styles["Normal"]
                )
            )
            story.append(
                Paragraph(
                    a["comentario"], ParagraphStyle("C", leftIndent=20, fontSize=9)
                )
            )
            story.append(Spacer(1, 5))
    ...
    def create_report_resolution_pdf(
        self, reporte: dict, seguimiento: list
    ) -> io.BytesIO:
        ...
        story.append(
            Paragraph(
                f"Por la presente se informa que el reporte ID #{reporte['id'][:8]}, categorizado como <b>{reporte['categoria']}</b> y ubicado en <b>{reporte['ubicacion_texto']}</b>, ha sido dado por FINALIZADO.",
                styles["Normal"],
            )
        )
        ...
        for s in seguimiento:
            if s["comentario_publico"]:
                story.append(
                    Paragraph(
                        f"• {s['fecha'][:10]}: {s['comentario_publico']}",
                        styles["Normal"],
                    )
                )
        ...
        story.append(
            Paragraph(
                "__________________________<br/>Comisión Directiva<br/>Consorcio Canalero 10 de Mayo",
    ...
    def create_general_impact_report_pdf(self, data: dict) -> io.BytesIO:
        ...
        # Satélite
        story.append(
            Paragraph("1. Estado de Cuencas (Análisis Satelital)", styles["Heading2"])
        )
        ...
        for cuenca, info in data["cuencas_data"].items():
            story.append(
                Paragraph(f"<b>Cuenca {cuenca.capitalize()}</b>", styles["Normal"])
            )
            story.append(
                Paragraph(
                    f"• Reportes activos: {info['reportes_count']}", styles["Normal"]
                )
            )
            story.append(
                Paragraph(
                    f"• Sugerencias recibidas: {info['sugerencias_count']}",
                    styles["Normal"],
                )
            )
            story.append(Spacer(1, 5))
