...
describe('useGEELayers', () => {
  ...
  describe('AGGRESSIVE MUTATION KILLERS - Filter & Map Operations', () => {
    it('kills: layersArray should filter out undefined values', async () => {
      ...
      // Only zona should be in layersArray (candil failed)
      expect(result.current.layersArray.length).toBeGreaterThan(0);
      expect(result.current.layersArray.some(l => l.name === 'zona')).toBe(true);
    });

    it('kills: layersArray should map entries to objects with name and data', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        json: async () => mockGeoJSON,
      });
      ...
      expect(result.current.layersArray.length).toBeGreaterThan(0);
      result.current.layersArray.forEach((item) => {
        expect(item).toHaveProperty('name');
        expect(item).toHaveProperty('data');
        expect(item.data).toHaveProperty('type', 'FeatureCollection');
        expect(item.data).toHaveProperty('features');
      });
    });

    it('kills: layersArray map must create objects with FeatureCollection data', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        json: async () => mockGeoJSON,
      });
      ...
      expect(result.current.layersArray.length).toBeGreaterThan(0);
      expect(result.current.layersArray[0].data.type).toBe('FeatureCollection');
      expect(Array.isArray(result.current.layersArray[0].data.features)).toBe(true);
    });

    it('kills: empty layers should result in empty layersArray', () => {
      const { result } = renderHook(() => useGEELayers({ enabled: false }));
    ...
    it('kills: filter must preserve data !== undefined check', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        json: async () => mockGeoJSON,
      });
      ...
      // Should have at least the successful layer
      expect(result.current.layersArray.length).toBeGreaterThan(0);
    });
  });
  ...
  describe('AGGRESSIVE MUTATION KILLERS - Load Layer Edge Cases', () => {
    it('kills: loadLayer should return [name, null] when response.ok is false', async () => {
      mockFetch.mockResolvedValue({
        ok: false,
        status: 404,
      });
    ...
    it('kills: loadLayer should return [name, null] when parseFeatureCollection returns null', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        json: async () => ({ invalid: 'structure' }),
      });
    ...
    it('kills: loadLayer catch should return [name, null] on network error', async () => {
      mockFetch.mockRejectedValue(new Error('Network error'));
    ...
    it('kills: loadedCount should increment only when data is not null', async () => {
      mockFetch.mockImplementation((url: string) => {
        if (url.includes('zona')) {
          return Promise.resolve({
            ok: true,
      ...
      // Only zona loaded successfully
      expect(Object.keys(result.current.layers).length).toBeGreaterThan(0);
      expect(result.current.error).toBe('No se pudieron cargar las capas del mapa');
    });

    it('kills: increment operator must be ++ not --', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        json: async () => mockGeoJSON,
      });
    ...
    it('kills: all layers should load when all responses are ok', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        json: async () => mockGeoJSON,
      });
      ...
      const { result } = renderHook(() =>
        useGEELayers({ layerNames: ['zona', 'candil', 'ml'], enabled: true })
      );
    ...
    it('kills: partial layer load (some succeed, some fail)', async () => {
      mockFetch.mockImplementation((url: string) => {
        if (url.includes('zona') || url.includes('candil')) {
          return Promise.resolve({
            ok: true,
            json: async () => mockGeoJSON,
          });
  ...
  describe('AGGRESSIVE MUTATION KILLERS - Comparison Logic & Boundaries', () => {
    ...
    it('kills: when loadedCount = 1 and layerNames.length = 1, no error', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        json: async () => mockGeoJSON,
      });
  ...
  describe('AGGRESSIVE MUTATION KILLERS - Error Message Precision', () => {
    ...
    it('kills: catch block error must be "Error al cargar capas del mapa"', async () => {
  ...
  describe('AGGRESSIVE MUTATION KILLERS - API URL Construction', () => {
    ...
    it('kills: fetch is called when enabled=true', async () => {
      mockFetch.mockClear();
      mockFetch.mockResolvedValue({
        ok: true,
        json: async () => mockGeoJSON,
  ...
  describe('AGGRESSIVE MUTATION KILLERS - Initial State', () => {
    ...
    it('kills: enabled default must be true and trigger fetch', async () => {
      mockFetch.mockClear();
      mockFetch.mockResolvedValue({
        ok: true,
        json: async () => mockGeoJSON,
  ...
  describe('AGGRESSIVE MUTATION KILLERS - Response Handling', () => {
    it('kills: response.ok true branch must process data', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        json: async () => mockGeoJSON,
      });
    ...
    it('kills: when validatedData is valid, must add to layers', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        json: async () => mockGeoJSON,
      });
