 ...
 * disclosure control, wherever the result is read. This suite asserts it
 * STRUCTURALLY, in the rendered DOM:
 *
 *   - the exact copy is present;
 *   - it renders in BOTH surfaces (ranked list and crossing popup);
...
describe('RoadFlowDisclaimer', () => {
  ...
  it('renders TWICE — once per surface, both mounted at the same time', () => {
    renderWithMantine(
      <>
        <RoadFlowDisclaimer surface="lista" />
        <RoadFlowDisclaimer surface="popup" />
      </>
    );
    const nodes = screen.getAllByText(/no una medición hidráulica/);
    expect(nodes.length).toBe(2);
    expect(screen.getByTestId(`${ROAD_FLOW_DISCLAIMER_TEST_ID}-lista`)).toBeTruthy();
    expect(screen.getByTestId(`${ROAD_FLOW_DISCLAIMER_TEST_ID}-popup`)).toBeTruthy();
  });
  ...
  it('is visible immediately: no hidden attribute, no display:none', () => {
    renderWithMantine(<RoadFlowDisclaimer surface="popup" />);
    const node = screen.getByTestId(`${ROAD_FLOW_DISCLAIMER_TEST_ID}-popup`);
