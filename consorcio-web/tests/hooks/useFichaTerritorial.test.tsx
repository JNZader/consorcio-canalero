...
describe('useFichaTerritorial', () => {
  ...
  it('POSTs analisis-zona and returns the parsed ficha', async () => {
    ...
    expect(JSON.parse(String(init?.body))).toEqual({ tipo: 'parcela', nomenclatura: '13-06-01' });
  });
  ...
  it.each([
    ...
    [422, 'cap_excedido'],
    [429, 'limite_de_tasa'],
  ])('does NOT retry a terminal %i (fetched exactly once)', async (status, codigo) => {
    fetchMock.mockResolvedValue(jsonResponse(status, { detail: 'nope', codigo }));
  ...
  it('retries a transient 503 exactly once (fetched twice)', async () => {
    fetchMock.mockResolvedValue(
      jsonResponse(503, { detail: 'dataset off', codigo: 'dataset_no_cargado' })
    );
  ...
  it('preserves the HTTP status and codigo on a failure', async () => {
    fetchMock.mockResolvedValue(
      jsonResponse(404, { detail: 'No existe una parcela', codigo: 'parcela_no_encontrada' })
    );
