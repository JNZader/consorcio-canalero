...
export function ImageExplorerMap({ mapRef, loading, resultExists, error }: ImageExplorerMapProps) {
  return (
    <>
      ...
      <Card padding={0} radius="md" withBorder style={{ minHeight: 450, position: 'relative', flex: '1 1 auto' }}>
        <div ref={mapRef} style={{ width: '100%', height: 450, borderRadius: 'var(--mantine-radius-md)' }} />
        {loading && (
        ...
        {!resultExists && !loading && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
            <Paper p="lg" radius="md" shadow="sm" style={{ pointerEvents: 'auto', textAlign: 'center' }}>
              ...
              <Text c="dimmed" size="sm">Selecciona un dia con imagenes</Text>
              <Text c="dimmed" size="xs">del calendario para previsualizar</Text>
            </Paper>
          </div>
        )}
      </Card>
